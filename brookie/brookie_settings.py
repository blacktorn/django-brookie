...
BROOKIE_SAVE_PATH = getattr(settings, 'BROOKIE_SAVE_PATH', os.path.join(
    settings.MEDIA_ROOT, 'brookie/invoices/'))

# Amount of days before an invoice expires.
INVOICE_EXPIRATION_DAYS = getattr(settings, 'INVOICE_EXPIRATION_DAYS', 30)
...
INVOICE_ID_FORMAT = getattr(settings, 'INVOICE_ID_FORMAT', '{prefix}{year}{unique_id}')

# Default hourly rate
INVOICE_HOURLY_RATE = getattr(settings, 'INVOICE_HOURLY_RATE', 50.00)

# Invoice status choices.
INVOICE_STATUS_CHOICES = getattr(settings, 'INVOICE_STATUS_CHOICES', (
    (1, ugettext('Under development')),
    (2, ugettext('Sent')),
    (3, ugettext('Reminded')),
    (4, ugettext('Paid'))
))
...
INVOICE_CURRENCY_CHOICES = getattr(settings, 'INVOICE_CURRENCY_CHOICES', (
    ('euro', ugettext('Euro')),
    ('gbp', ugettext('Pound')),
...
QUOTE_STATUS_CHOICES = getattr(settings, 'QUOTE_STATUS_CHOICES', (
    (1, ugettext('Draft')),
    (2, ugettext('Sent')),
    (3, ugettext('Declined')),
    (4, ugettext('Accepted'))
